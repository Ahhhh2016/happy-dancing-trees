...
void ARAP::initShapeWithTexture(const std::vector<Eigen::Vector3f> &V,
...
void ARAP::move(int vertex, Vector3f targetPosition)
{
    ...
    // Warm-start from the last accepted ARAP-L pose so the inner loop converges fast.
    std::vector<Eigen::Vector3f> new_vertices =
        (m_hasArapL && m_gCurrent.size() == m_p.size()) ? m_gCurrent : m_p;
    ...
    for (int iter = 0; iter < num_iterations; iter++) {
        ...
        if (m_hasArapL) {
            ...
            new_vertices[vertex] = targetPosition;
            for (int a : anchors) if (a != vertex) new_vertices[a] = p[a];
        }
    }

    if (m_hasArapL) m_gCurrent = new_vertices;
