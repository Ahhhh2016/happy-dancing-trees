#include "buildlayerconstraints.h"

buildLayerConstraints::buildLayerConstraints() {}
...
#include <Eigen/Dense>
 ...
 std::vector<LayerConstraint> buildLayerConstraints::buildiLayerConstraints(
...
// ============================================================
//  HOW TO CALL THIS  (add to loadMeshFromFile in glwidget.cpp,
//  right after the m_arap.init(...) / m_arap.initTexture(...) call):
//
//    // Build and install layering constraints so ARAP-L separates
//    // overlapping parts by depth order.
//    auto layerConstraints = buildLayerConstraints(vertices, mesh);
//    m_arap.setLayerConstraints(layerConstraints);
//    std::cout << "Layer constraints: " << layerConstraints.size() << std::endl;
//
//  NOTE: For this to work, `mesh` (the StitchedMesh) must be available
//  inside loadMeshFromFile.  Currently the mesh is built in
//  MainWindow::buildMeshAndSaveObj() and only the OBJ path is passed to
//  GLWidget.  You have two options:
//
//  Option A (recommended):  Pass the StitchedMesh from MainWindow to
//  GLWidget alongside the OBJ path.  Add a new method:
//
//    void GLWidget::setMeshData(const std::string &path,
//                               const StitchedMesh &mesh);
//
//  and call buildLayerConstraints() there.
//
//  Option B (simpler but less accurate):  Store the StitchedMesh in
//  MainWindow as m_lastMesh and expose it via a getter, then call
//  buildLayerConstraints() in loadMeshFromFile() after the OBJ is loaded
//  and the float vertices are available.
//
//  The depth-order information lost during OBJ export (writeOBJWithPlanarUV
//  discards isDirichlet / sideFlags / depthOrder) is the core reason the
//  layering step currently does nothing — those flags are needed to know
//  which vertex should be in front of which.
// ============================================================
